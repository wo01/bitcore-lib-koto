...
Transaction.shallowCopy = function(transaction) {
  var copy = new Transaction(transaction.toBuffer(1));
  return copy;
};
...
var hashProperty = {
  ...
  enumerable: true,
  get: function() {
    if (this.version >= 4 && !this.isCoinbase()) {
      return new BufferReader(this._getHashwithoutSig()).readReverse().toString('hex');
    } else {
...
Object.defineProperty(Transaction.prototype, 'id', hashPropertywithSig);
...
Transaction.prototype._getHash = function() {
  return Hash.sha256sha256(this.toBuffer(1));
};

/**
 * Retrieve the little endian hash of the transaction (without ScriptSig)
 * @return {Buffer}
 */
Transaction.prototype._getHashwithoutSig = function() {
  return Hash.sha256sha256(this.toBuffer(0));
};
...
Transaction.prototype.uncheckedSerialize = Transaction.prototype.toString = function() {
  return this.toBuffer(1).toString('hex');
};
...
Transaction.prototype.toBuffer = function(withSig) {
  var writer = new BufferWriter();
  return this.toBufferWriter(writer, withSig).toBuffer();
};

Transaction.prototype.toBufferWriter = function(writer, withSig) {
  ...
  _.each(this.inputs, function(input) {
    input.toBufferWriter(writer, withSig);
  });
...
Transaction.prototype._newTransaction = function() {
  ...
  this.fSaplinged = true;
  this.nVersionGroupId = '0x9023E50A';
};
...
Transaction.prototype.verify = function() {
  ...
  // Size limits
  if (this.toBuffer(1).length > MAX_BLOCK_SIZE) {
